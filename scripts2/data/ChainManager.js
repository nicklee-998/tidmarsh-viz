...
//	date -> timestamp
//----------------------------------------------
ChainManager.prototype.fetchData = function(did, sid, date)
{
	var dat = this._dFactory.findData(did, sid, date);
	return dat;
}

ChainManager.prototype.fetchDataById = function(did, sid)
{
...
//----------------------------------------------
/*ChainManager.prototype.fetchDataByBlock = function(did, sid, perc)
 {
...
//---------------------------------------------------------------------
ChainManager.prototype.fetchMultiDevicesByDate = function(dlist, slist, start, end)
{
	// --------------------------------
...
ChainManager.prototype._startDeviceList = function()
{
...
ChainManager.prototype._onDeviceComplete = function()
{
...
// 	start&end is object -> {year, month, day}
//---------------------------------------------------------------------
ChainManager.prototype.fetchDeviceDataByDate = function(deviceid, sensors, start, end)
{
...
ChainManager.prototype._startSensorsList = function()
{
...
ChainManager.prototype._startSensorDataChain = function()
{
	if(this._expired) {
		// go on next sensor...
		this._loadIdx2++;
 ...
 * Rollup Data
 *****************************************/
ChainManager.prototype._rollupData = function(rawdata, type)
{
	//console.log(rawdata);
	// 目前默认平均值
...
ChainManager.prototype._rollupUnit = function(u)
{
	var nu = u;
	if(u == "percent")
		nu = "%";
...
ChainManager.prototype.getDeviceBySensor = function(sensor_url)
{
	var device = null;
